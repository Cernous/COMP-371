...
int main(int argc, char*argv[])
{
    ...
    GLuint shaderShadow = loadSHADER(shaderPathPrefix + "shadow_vertex.phong.glsl",
                                   shaderPathPrefix + "shadow_fragment.phong.glsl");
    ...
    // [ADDED] Static texture unit binding for shadow samplers
    glUseProgram(shaderScene);
    glUniform1i(glGetUniformLocation(shaderScene, "shadowMap"), 1);
    glUseProgram(textureScene);
    glUniform1i(glGetUniformLocation(textureScene, "shadowMap"), 1);
    glUseProgram(0);
    ...
    while(!glfwWindowShouldClose(window))
    {
        ...
        float emitterStrengths[2] = { 5.0f, 5.0f }; // TO CHANGE INTENSITY
        glm::vec3 emitterColors[2] = { glm::vec3(1.0f), glm::vec3(1.0f) };

        // Build light space matrix from the first emitter looking at scene origin
        glm::mat4 lightProj = glm::perspective(glm::radians(90.0f), 1.0f, 0.1f, 50.0f);
        glm::mat4 lightView = glm::lookAt(emitterPositions[0], glm::vec3(0.0f, 0.0f, 0.0f), glm::vec3(0.0f, 1.0f, 0.0f));
        ...
        auto setShadowWorld = [&](const glm::mat4& M){
            GLuint loc = glGetUniformLocation(shaderShadow, "transform_in_light_space");
            glm::mat4 m = lightSpaceMatrix * M;
            glUniformMatrix4fv(loc, 1, GL_FALSE, &m[0][0]);
        };

        glBindVertexArray(vao);
        // Center cube
        setShadowWorld(glm::mat4(1.0f));
        glDrawArrays(GL_TRIANGLES, 0, 36);
        // Light cubes (optional as occluders)
        setShadowWorld(cubeTop);
        glDrawArrays(GL_TRIANGLES, 0, 36);
        setShadowWorld(cubeRight);
        glDrawArrays(GL_TRIANGLES, 0, 36);
        // Textured meshes
        setShadowWorld(glm::translate(glm::mat4(1.0f), glm::vec3(0.0f, -0.5f, 0.0f)) * glm::scale(glm::mat4(1.0f), glm::vec3(2.5f)));
        glBindVertexArray(tableVAO);
        glDrawArrays(GL_TRIANGLES, 0, tableVertices);

        setShadowWorld(glm::translate(glm::mat4(1.0f), glm::vec3(0.0f, 1.3f, 0.0f)) *
                       glm::rotate(glm::mat4(1.0f), glm::radians(spinningObjAngle), glm::vec3(0.0f, 1.0f, 0.0f)) *
        ...
        // Use shaderScene to draw cubes
        glUseProgram(shaderScene);

        // Set per-frame uniforms
        glUniformMatrix4fv(glGetUniformLocation(shaderScene, "lightSpaceMatrix"), 1, GL_FALSE, &lightSpaceMatrix[0][0]);
        ...
        glUniform3fv(glGetUniformLocation(shaderScene, "emitterColor"), 2, &emitterColors[0].x);
        glUniform1i(glGetUniformLocation(shaderScene, "numEmitters"), 2);
        glUniform3f(glGetUniformLocation(shaderScene, "viewPos"), cameraPosition.x, cameraPosition.y, cameraPosition.z);

        // Draw geometry
        glBindVertexArray(vao);

        // Center cube (static)
        glUniform1i(glGetUniformLocation(shaderScene, "useOverride"), GL_FALSE);
        setWorldMatrix(shaderScene, glm::mat4(1.0f));
        glDrawArrays(GL_TRIANGLES, 0, 36);
        ...
        // A separate colored cube
        glUniform1i(glGetUniformLocation(shaderScene, "useOverride"), GL_FALSE);
        setWorldMatrix(shaderScene,
        ...
        glUniformMatrix4fv(glGetUniformLocation(textureScene, "lightSpaceMatrix"), 1, GL_FALSE, &lightSpaceMatrix[0][0]);
        ...
        glUniform3fv(glGetUniformLocation(textureScene, "emitterColor"), 2, &emitterColors[0].x);
        ...
        setWorldMatrix(textureScene, glm::translate(glm::mat4(1.0f), glm::vec3(0.0f, -0.5f, 0.0f)) * glm::scale(glm::mat4(1.0f), glm::vec3(2.5f)));
        glBindVertexArray(tableVAO);
        glDrawArrays(GL_TRIANGLES, 0, tableVertices);
        ...
        setWorldMatrix(textureScene, glm::translate(glm::mat4(1.0f), glm::vec3(0.0f, 1.3f, 0.0f)) *
                                      glm::rotate(glm::mat4(1.0f), glm::radians(spinningObjAngle), glm::vec3(0.0f, 1.0f, 0.0f)) *
                                      glm::scale(glm::mat4(1.0f), glm::vec3(0.7f)));
        glBindVertexArray(suzanneVAO);
        glDrawArrays(GL_TRIANGLES, 0, suzanneVertices);
