...
int main(int argc, char* argv[]) {
  ...
  while (!glfwWindowShouldClose(window)) {
    ...
    vec3 lightPosition =    vec3(0.0f,20.0f,10.0f); // the location of the light in 3D space: fixed position
        // * vec3(cosf(phi) * cosf(phi), sinf(phi),
        //      -cosf(phi) * sinf(phi)) *
        5.0f;  // variable position

    vec3 lightFocus(0, 0, 0);  // the point in 3D space the light "looks" at
